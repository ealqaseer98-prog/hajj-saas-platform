  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase.from('travellers').select('id, gender'),
    ...
    <div className="p-6 space-y-5" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">توزيع المسافرين حسب الجنس</h2>
          <div className="space-y-3">
            ...
            <div>
              <div className="flex items-center justify-between text-sm mb-1.5">
                ...
                <span className="font-bold text-blue-700">{stats?.males ?? 0}
            ...
            <div>
              <div className="flex items-center justify-between text-sm mb-1.5">
                ...
                <span className="font-bold text-pink-700">{stats?.females ?? 0}
            ...
            {(stats?.noGender ?? 0) > 0 && (
              <p className="text-xs text-gray-400">{stats?.noGender} مسافر بدون تحديد جنس</p>
            )}
            {/* Stacked visual */}
            {(stats?.total ?? 0) > 0 && (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="space-y-3">
            {[
              { label: 'موافق عليه',  count: stats?.visaApproved ?? 0, color: 'bg-green-400', textColor: 'text-green-700', bg: 'bg-green-50', icon: <CheckCircle2 size={14} /> },
              { label: 'في الانتظار', count: stats?.visaPending ?? 0,  color: 'bg-amber-400', textColor: 'text-amber-700',  bg: 'bg-amber-50', icon: <Clock size={14} /> },
              { label: 'مرفوض',       count: stats?.visaRejected ?? 0, color: 'bg-red-400',   textColor: 'text-red-700',   bg: 'bg-red-50',   icon: <AlertCircle size={14} /> },
            ].map(row => {
